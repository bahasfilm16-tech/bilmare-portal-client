...
import {
  ...
  CheckCircle2, Loader2, RefreshCw, X, AlertTriangle,
...
export const EngagementAdmin = () => {
  ...
  // Resend invite — update status + toast
  ...
  const handleArchival = async () => {
    ...
    try {
      const { error } = await supabase
        .from('projects')
        .update({ status: 'Archived', status_reason: 'Diarsipkan atas permintaan klien.' })
        .eq('id', project.id);
      if (error) throw error;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <Building className="w-5 h-5 text-indigo-600" /> Project Details
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-1">Project Name</p>
              <p className="text-sm font-medium text-slate-900">{project.name}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-1">Service Tier</p>
            ...
            <div>
              ...
              <Badge variant={project.status === 'At Risk' ? 'destructive' : 'success'} className="gap-1">
            ...
            <div className="pt-4 border-t border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-xs text-slate-500">OJK Deadline</p>
                  <p className="text-sm font-medium text-slate-900">
                    {project.deadlineOJK ? format(new Date(project.deadlineOJK), 'dd MMM yyyy') : '—'}
                  </p>
                </div>
                {project.rupsDate && (
                  <div className="col-span-2">
                    <p className="text-xs text-slate-500">RUPS Date</p>
                    <p className="text-sm font-medium text-slate-900">
                      {format(new Date(project.rupsDate), 'dd MMM yyyy')}
                    </p>
                  </div>
                )}
              </div>
            ...
            {project.scope && (
              <div className="pt-4 border-t border-slate-100">
                <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-1">Scope</p>
                    ...
                    <TableRow key={user.id}>
                      ...
                      <TableCell>
                        {user.status === 'Active' ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 py-1 px-2 rounded-md text-xs font-medium bg-amber-50 text-amber-700">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-500" /> Pending
                          </span>
                        )}
                      </TableCell>
                      <TableCell className="text-xs text-slate-500">
                        {user.lastAccess ? format(user.lastAccess, 'dd MMM yyyy, HH:mm') : '—'}
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => { setSettingsUser(user); setNewRole(user.role); }}
                            title="Ubah Role"
                          >
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <Shield className="w-5 h-5 text-indigo-600" /> Security & Compliance
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <div className="flex items-center justify-between p-4 border border-slate-200 rounded-xl bg-slate-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Wajib 2FA untuk semua anggota project.</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between p-4 border border-slate-200 rounded-xl bg-slate-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Kirim ringkasan harian update proyek.</p>
                    </div>
                  </div>
                  <div className="w-10 h-5 bg-indigo-500 rounded-full relative cursor-pointer">
                    <div className="w-4 h-4 bg-white rounded-full absolute right-0.5 top-0.5 shadow" />
              ...
              <div className="space-y-4">
                <div className="p-4 border border-slate-200 rounded-xl">
                  ...
                  <p className="text-xs text-slate-600 mb-4">
                    Data proyek akan diarsipkan secara aman 90 hari setelah proyek selesai. Final deliverables tetap dapat diakses.
                  </p>
                  <Button variant="outline" size="sm" className="w-full gap-2" onClick={() => setPolicyOpen(true)}>
                ...
                <div className="p-4 border border-red-200 bg-red-50 rounded-xl">
                  ...
                  <Button variant="destructive" size="sm" className="w-full" onClick={() => { setArchivalOpen(true); setArchivalConfirm(''); }}>
                    Request Project Archival
                  </Button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setSettingsUser(null)} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-3 bg-slate-50 rounded-lg p-3">
                <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold">
                  {settingsUser.name.split(' ').map(n => n[0]).join('').substring(0, 2)}
                </div>
                <div>
                  <p className="font-medium text-slate-900">{settingsUser.name}</p>
                  <p className="text-sm text-slate-500">{settingsUser.email}</p>
                </div>
              </div>
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Full Access: semua fitur · Review Only: hanya lihat · Submitter: hanya upload dokumen
                </p>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setPolicyOpen(false)} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 space-y-6 text-sm text-slate-700">
              {[
                { title: '1. Penyimpanan Data', content: 'Seluruh data proyek — termasuk dokumen, temuan, dan komunikasi — disimpan secara aman di infrastruktur Supabase dengan enkripsi at-rest dan in-transit menggunakan standar AES-256.' },
                ...
                { title: '3. Akses & Kontrol', content: 'Akses dikendalikan berbasis role (Full Access, Review Only, Document Submitter). Setiap akses tercatat dalam audit log. Tim Bilmare hanya dapat mengakses data yang relevan dengan proyek klien.' },
            ...
            <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex justify-end">
              <Button onClick={() => setPolicyOpen(false)}>Tutup</Button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-red-200 flex justify-between items-center bg-red-50">
              <h2 className="text-lg font-bold text-red-900 flex items-center gap-2">
                <AlertTriangle className="w-5 h-5" /> Request Project Archival
              </h2>
              <button onClick={() => setArchivalOpen(false)} className="p-2 text-red-400 hover:text-red-600 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-sm text-red-800 font-medium mb-2">⚠️ Tindakan ini tidak dapat dibatalkan!</p>
                <p className="text-xs text-red-700 leading-relaxed">
                  Mengarsipkan proyek akan menonaktifkan akses ke semua fitur portal. Data akan disimpan selama 90 hari sebelum dihapus permanen. Final deliverables tetap dapat diakses.
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Ketik <span className="font-bold text-red-600">ARSIPKAN</span> untuk konfirmasi:
                </label>
                <input
                  type="text"
                  value={archivalConfirm}
                  onChange={e => setArchivalConfirm(e.target.value)}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  ...
                  disabled={archivalConfirm !== 'ARSIPKAN' || archiving}
