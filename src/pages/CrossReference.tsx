...
export const CrossReference = () => {
  ...
  const handleExport = () => {
    ...
    const getRowStyle = (status: string, isEven: boolean) => ({
    ...
    const styleInconsistent = {
      font: { name: 'Calibri', sz: 10, bold: true, color: { rgb: 'DC2626' } },
      fill: { fgColor: { rgb: 'FEE2E2' } },
      alignment: { horizontal: 'center', vertical: 'center' },
    };
    const styleUnverified = {
      font: { name: 'Calibri', sz: 10, bold: true, color: { rgb: '6B7280' } },
    ...
    // Stats
    wsData.push([
      { v: 'TOTAL CLAIMS', s: { font: { name: 'Calibri', sz: 9, color: { rgb: '6B7280' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'F9FAFB' } } } },
      { v: 'CONSISTENT', s: { font: { name: 'Calibri', sz: 9, color: { rgb: '6B7280' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'F9FAFB' } } } },
      { v: 'INCONSISTENT', s: { font: { name: 'Calibri', sz: 9, color: { rgb: '6B7280' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'F9FAFB' } } } },
      { v: 'UNVERIFIED', s: { font: { name: 'Calibri', sz: 9, color: { rgb: '6B7280' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'F9FAFB' } } } },
      '', '', '', '',
    ]);
    wsData.push([
      { v: claims.length, s: { font: { name: 'Calibri', sz: 14, bold: true, color: { rgb: '4F46E5' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'EEF2FF' } }, border: { top: { style: 'thin', color: { rgb: 'C7D2FE' } }, bottom: { style: 'thin', color: { rgb: 'C7D2FE' } }, left: { style: 'thin', color: { rgb: 'C7D2FE' } }, right: { style: 'thin', color: { rgb: 'C7D2FE' } } } } },
      { v: consistentCount, s: { font: { name: 'Calibri', sz: 14, bold: true, color: { rgb: '047857' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'D1FAE5' } }, border: { top: { style: 'thin', color: { rgb: 'A7F3D0' } }, bottom: { style: 'thin', color: { rgb: 'A7F3D0' } }, left: { style: 'thin', color: { rgb: 'A7F3D0' } }, right: { style: 'thin', color: { rgb: 'A7F3D0' } } } } },
      { v: inconsistentCount, s: { font: { name: 'Calibri', sz: 14, bold: true, color: { rgb: 'DC2626' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'FEE2E2' } }, border: { top: { style: 'thin', color: { rgb: 'FECACA' } }, bottom: { style: 'thin', color: { rgb: 'FECACA' } }, left: { style: 'thin', color: { rgb: 'FECACA' } }, right: { style: 'thin', color: { rgb: 'FECACA' } } } } },
      { v: unverifiedCount, s: { font: { name: 'Calibri', sz: 14, bold: true, color: { rgb: '6B7280' } }, alignment: { horizontal: 'center' }, fill: { fgColor: { rgb: 'F3F4F6' } }, border: { top: { style: 'thin', color: { rgb: 'E5E7EB' } }, bottom: { style: 'thin', color: { rgb: 'E5E7EB' } }, left: { style: 'thin', color: { rgb: 'E5E7EB' } }, right: { style: 'thin', color: { rgb: 'E5E7EB' } } } } },
      '', '', '', '',
    ]);
    wsData.push(['', '', '', '', '', '', '', '']);
    ...
    wsData.push([
      { v: 'ID', s: styleHeader },
      { v: 'Claim in Draft', s: styleHeader },
      { v: 'Location', s: styleHeader },
      { v: 'Category', s: styleHeader },
      { v: 'Source Document', s: styleHeader },
      { v: 'Source Page', s: styleHeader },
      { v: 'Status', s: styleHeader },
      { v: 'Verifier', s: styleHeader },
    ]);

    // Data rows
    claims.forEach((c, idx) => {
      const isEven = idx % 2 === 0;
      const base = getRowStyle(c.status, isEven);
      const statusStyle = c.status === 'Consistent' ? styleConsistent
        : c.status === 'Inconsistent' ? styleInconsistent
        : styleUnverified;

      wsData.push([
        { v: c.id, s: { ...base, font: { ...base.font, bold: true } } },
        { v: c.claim, s: base },
        { v: c.report, s: { ...base, alignment: { horizontal: 'center', vertical: 'top' } } },
        { v: c.category, s: { ...base, alignment: { horizontal: 'center', vertical: 'top' } } },
        { v: c.sourceDoc, s: base },
        { v: c.sourcePage, s: { ...base, alignment: { horizontal: 'center', vertical: 'top' } } },
        { v: c.status, s: statusStyle },
        { v: c.verifier, s: base },
      ]);
    });
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-700 mt-1">
              Found {inconsistentCount} claims that do not match the verified source documents. Please review and resolve.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500">Consistent</p>
              <p className="text-2xl font-bold text-emerald-600">{consistentCount}</p>
            </div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500">Inconsistent</p>
              <p className="text-2xl font-bold text-red-600">{inconsistentCount}</p>
            </div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-500">{unverifiedCount}</p>
            </div>
      ...
      <Card>
        <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <Button variant="ghost" onClick={() => { setSearchTerm(''); setCategoryFilter('All'); setStatusFilter('All'); }}>
              Reset
            </Button>
          </div>
        </CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-1/3">Claim in Draft</TableHead>
            ...
            <TableBody>
              {filteredClaims.map((claim) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant="outline" className="bg-white">{claim.report}</Badge>
                  </TableCell>
                  <TableCell>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-xs text-slate-500">Verified by {selectedClaim.verifier}</p>
                <p className="text-xs text-slate-500">{format(selectedClaim.verificationDate, 'dd MMM yyyy')}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-slate-900 flex items-center gap-2">
                  <FileText className="w-4 h-4" /> Draft Report ({selectedClaim.report})
                </h4>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                  <p className="text-sm text-slate-800 italic leading-relaxed">"{selectedClaim.claim}"</p>
                  <div className="mt-4 pt-4 border-t border-slate-200">
                    <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-1">Extracted Value</p>
                    <p className="text-lg font-bold text-slate-900">{selectedClaim.currentYearValue}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className={`border rounded-lg p-4 ${selectedClaim.status === 'Inconsistent' ? 'bg-red-50 border-red-200' : selectedClaim.status === 'Consistent' ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200/50">
                    <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-1">Verified Value</p>
                    <p className={`text-lg font-bold ${selectedClaim.status === 'Inconsistent' ? 'text-red-700' : 'text-emerald-700'}`}>
                      {selectedClaim.currentYearValue}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Last Year (2023)</TableHead>
                    <TableHead>Current Year (2024)</TableHead>
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell>
                      {selectedClaim.lastYearValue}
                      <span className="block text-[10px] text-slate-500">{selectedClaim.lastYearSource}</span>
                    </TableCell>
                    <TableCell className={selectedClaim.status === 'Inconsistent' ? 'text-red-600 font-medium' : 'text-emerald-700 font-medium'}>
                      {selectedClaim.currentYearValue}
                    </TableCell>
