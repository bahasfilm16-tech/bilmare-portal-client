...
export const Deliverables = () => {
  ...
  const avgProgress = totalAll > 0
    ? Math.round(deliverables.reduce((sum, d) => sum + d.progress, 0) / totalAll)
    : 0;
  ...
  const handleDownload = async (deliverable: Deliverable) => {
    if (!deliverable.filePath) {
      addToast('File belum tersedia untuk diunduh.', 'error');
      return;
    }
    ...
    try {
      const { data, error } = await supabase.storage
        .from('Portal Client')
        .createSignedUrl(deliverable.filePath, 120);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500">Ready to Download</p>
              <p className="text-2xl font-bold text-emerald-600">{readyDeliverables.length}</p>
            </div>
      ...
      <div className="flex gap-4 border-b border-slate-200">
        ...
        >
          Ready for Download ({readyDeliverables.length})
        </button>
        <button
          className={`pb-3 px-4 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'progress'
        ...
        >
          In Progress ({inProgressDeliverables.length})
        </button>
      </div>
                      ...
                      <div className="flex items-center gap-3 mt-2 flex-wrap">
                        ...
                        {deliverable.dateAvailable && (
                          <span className="text-xs text-slate-400">
                            Tersedia: {format(deliverable.dateAvailable, 'dd MMM yyyy')}
                          </span>
                        )}
                      </div>
                      <div className="mt-3">
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-1.5">
                          <div
                            className={`h-1.5 rounded-full transition-all ${getProgressColor(deliverable.progress)}`}
                  ...
                  <div className="shrink-0 flex flex-col items-end gap-2">
                    {deliverable.status === 'Ready for Download' && deliverable.filePath ? (
                      ...
                      <div className="text-right">
                        <span className="text-xs text-amber-600 font-medium bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 block">
                          ⏳ Menunggu file<br />dari tim Bilmare
                        </span>
                      </div>
                    ) : (
                      <div className="text-right">
      ...
      {/* Upload Modal — untuk tim Bilmare */}
      {uploadingFor && (
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-slate-600 font-medium">
                {deliverables.find(d => d.id === uploadingFor)?.name}
              </p>
              <input
                ref={fileInputRef}
                type="file"
              ...
              >
                {uploadFile ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-slate-400 mt-1">PDF, Excel, Word — maks 50MB</p>
                  </>
                )}
              </div>
