    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col space-y-4">
      ...
      <div className="flex-1 flex overflow-hidden rounded-xl border border-slate-200 shadow-sm bg-white">
        ...
        <div className="flex-1 flex flex-col">
          {/* Header */}
          <div className="h-16 border-b border-slate-200 px-6 flex items-center shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-slate-900">Project Channel</h2>
                <p className="text-xs text-slate-500">Tim Bilmare & {project?.name?.split('—')[0]?.trim() ?? 'Klien'}</p>
              </div>
            </div>
          </div>
                ...
                <div key={msg.id} className={`flex gap-3 ${isClient ? 'flex-row-reverse' : ''}`}>
                  {showAvatar ? (
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 mt-1 ${
                      isClient ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-white'
                    }`}>
                      {isClient ? 'ME' : 'B'}
                    </div>
                    ...
                    {showAvatar && (
                      <div className="flex items-baseline gap-2 mb-1">
                        <span className="text-sm font-medium text-slate-900">
                          {isClient ? 'Anda' : 'Tim Bilmare'}
                        </span>
                        <span className="text-xs text-slate-400">
                          {format(msg.timestamp instanceof Date ? msg.timestamp : new Date(msg.timestamp), 'HH:mm')}
                        </span>
                      </div>
                    )}
          ...
          <div className="p-4 bg-white border-t border-slate-200 shrink-0">
            ...
            <p className="text-[10px] text-slate-400 mt-1.5 text-center">Enter untuk kirim · Shift+Enter untuk baris baru</p>
          </div>
        </div>
      </div>
    </div>
