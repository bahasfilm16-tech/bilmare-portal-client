...
export const GapRegister = () => {
  ...
  const handleSaveResponse = async () => {
    if (!clientResponse.trim()) {
      addToast('Response tidak boleh kosong', 'error');
      return;
    }
    ...
    try {
      const { error } = await supabase
        .from('gap_findings')
        .update({
          client_response: clientResponse.trim(),
  ...
  const handleExport = () => {
    ...
    const styleRowEven = (riskLevel: string) => ({
    ...
    const styleRowOdd = (riskLevel: string) => ({
      font: { name: 'Calibri', sz: 10, color: { rgb: '1F2937' } },
    ...
    const styleRiskMedium = {
      font: { name: 'Calibri', sz: 10, bold: true, color: { rgb: 'FFFFFF' } },
      fill: { fgColor: { rgb: 'D97706' } },
      alignment: { horizontal: 'center', vertical: 'center' },
      border: { bottom: { style: 'thin', color: { rgb: 'B45309' } } },
    };
    const styleRiskLow = {
      font: { name: 'Calibri', sz: 10, bold: true, color: { rgb: 'FFFFFF' } },
    ...
    const styleStats = {
      font: { name: 'Calibri', sz: 11, bold: true, color: { rgb: '4F46E5' } },
      fill: { fgColor: { rgb: 'EEF2FF' } },
      alignment: { horizontal: 'center', vertical: 'center' },
      border: {
    ...
    // Row 4-5: Summary stats
    wsData.push([
      { v: 'TOTAL FINDINGS', s: styleStatsLabel },
      { v: 'HIGH RISK', s: styleStatsLabel },
      { v: 'OPEN', s: styleStatsLabel },
      { v: 'RESOLVED', s: styleStatsLabel },
      '', '', '', '', '', '',
    ]);
    wsData.push([
      { v: stats.total, s: styleStats },
      { v: stats.highRisk, s: { ...styleStats, font: { ...styleStats.font, color: { rgb: 'DC2626' } } } },
      { v: stats.open, s: { ...styleStats, font: { ...styleStats.font, color: { rgb: 'D97706' } } } },
      { v: stats.resolved, s: { ...styleStats, font: { ...styleStats.font, color: { rgb: '059669' } } } },
      '', '', '', '', '', '',
    ]);
    // Row 6: Empty
    wsData.push(['', '', '', '', '', '', '', '', '', '']);
    ...
    wsData.push([
      { v: 'ID', s: styleHeader },
      { v: 'Risk Level', s: styleHeader },
      { v: 'Report', s: styleHeader },
      { v: 'Section', s: styleHeader },
      { v: 'Description', s: styleHeader },
      { v: 'Type', s: styleHeader },
      { v: 'Status', s: styleHeader },
      { v: 'Date Found', s: styleHeader },
      { v: 'Details', s: styleHeader },
      { v: 'Recommendation', s: styleHeader },
    ]);

    // Rows 8+: Data
    gapFindings.forEach((f, idx) => {
      const isEven = idx % 2 === 0;
      const baseStyle = isEven ? styleRowEven(f.riskLevel) : styleRowOdd(f.riskLevel);

      const riskStyle = f.riskLevel === 'High' ? styleRiskHigh
        : f.riskLevel === 'Medium' ? styleRiskMedium
        : styleRiskLow;

      const statusStyle = f.status === 'Open' ? styleStatusOpen
        : f.status === 'Client Acknowledged' ? styleStatusAck
        : f.status === 'In Resolution' ? styleStatusResolution
        : styleStatusResolved;

      wsData.push([
        { v: f.id, s: { ...baseStyle, font: { ...baseStyle.font, bold: true } } },
        { v: f.riskLevel, s: riskStyle },
        { v: f.report, s: { ...baseStyle, alignment: { horizontal: 'center', vertical: 'top' } } },
        { v: f.section ?? '', s: baseStyle },
        { v: f.description, s: baseStyle },
        { v: f.type, s: baseStyle },
        { v: f.status, s: statusStyle },
        { v: format(f.dateFound, 'dd MMM yyyy'), s: { ...baseStyle, alignment: { horizontal: 'center', vertical: 'top' } } },
        { v: f.detail ?? '', s: baseStyle },
        { v: f.recommendation ?? '', s: baseStyle },
      ]);
    });
    ...
    ws['!rows'] = [
      ...
      { hpt: 8 },  // Empty
      { hpt: 36 }, // Header
      ...gapFindings.map(() => ({ hpt: 60 })), // Data rows
    ];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <Button variant="ghost" onClick={() => { setSearchTerm(''); setStatusFilter('All'); setRiskFilter('All'); }}>
              Reset
            </Button>
          </div>
        </CardContent>
          ...
          <TableBody>
            {filteredFindings.map((finding) => (
              ...
              >
                ...
                <TableCell className="max-w-xs truncate" title={finding.description}>
                  {finding.description}
                </TableCell>
                <TableCell className="text-sm text-slate-600">{finding.type}</TableCell>
      ...
      <Modal
        ...
        onClose={() => { setSelectedFinding(null); setClientResponse(''); }}
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex gap-2">
                {getRiskBadge(selectedFinding.riskLevel)}
                {getStatusBadge(selectedFinding.status)}
              </div>
              <span className="text-sm text-slate-500">
                Found: {format(selectedFinding.dateFound, 'dd MMM yyyy')}
              </span>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500">
                ...
                <span>•</span>
                <span>{selectedFinding.section}</span>
                <span>•</span>
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-4">
              <h4 className="text-sm font-semibold text-indigo-900 mb-2 flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" /> Bilmare Recommendation
              </h4>
            ...
            {selectedFinding.status !== 'Resolved' && (
              <div className="border-t border-slate-200 pt-6">
                <h4 className="text-sm font-semibold text-slate-900 mb-3 flex items-center gap-2">
                  <MessageSquare className="w-4 h-4" /> Client Response
                </h4>
                <textarea
                ...
                <div className="flex justify-end gap-2">
                  {selectedFinding.status === 'Client Acknowledged' && (
                    <Button
                      variant="outline"
                      onClick={async () => {
                        await updateFindingStatus(selectedFinding.id, 'In Resolution');
                        setSelectedFinding({ ...selectedFinding, status: 'In Resolution' });
                      }}
                    >
            ...
            <div className="border-t border-slate-200 pt-6">
              <h4 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
                <History className="w-4 h-4" /> Resolution History
              </h4>
              <div className="space-y-4 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px before:h-full before:w-0.5 before:bg-slate-200">
                <div className="relative flex items-start gap-4">
                  <div className="w-4 h-4 rounded-full bg-red-500 ring-4 ring-white z-10 shrink-0 mt-1" />
                  <div>
                    <p className="text-sm font-medium text-slate-900">Finding Opened</p>
                    <p className="text-xs text-slate-500">By Bilmare Team • {format(selectedFinding.dateFound, 'dd MMM yyyy')}</p>
                  </div>
                </div>
                {selectedFinding.status !== 'Open' && (
                  <div className="relative flex items-start gap-4">
                    <div className="w-4 h-4 rounded-full bg-amber-500 ring-4 ring-white z-10 shrink-0 mt-1" />
                    <div>
                      <p className="text-sm font-medium text-slate-900">Client Acknowledged</p>
                      <p className="text-xs text-slate-500">By Client • {format(new Date(), 'dd MMM yyyy')}</p>
                    </div>
                  </div>
                )}
                {selectedFinding.status === 'Resolved' && (
                  <div className="relative flex items-start gap-4">
                    <div className="w-4 h-4 rounded-full bg-emerald-500 ring-4 ring-white z-10 shrink-0 mt-1" />
                    <div>
                      <p className="text-sm font-medium text-slate-900">Resolved</p>
                      <p className="text-xs text-slate-500">By Bilmare Team • {format(new Date(), 'dd MMM yyyy')}</p>
                    </div>
                  </div>
                )}
              </div>
