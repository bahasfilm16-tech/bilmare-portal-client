...
export const TimeSeries = () => {
  ...
  // Ambil nilai dari field `data` (dari Supabase)
  const getValue = (metric: any, year: string): number => {
    return metric.data?.[year] ?? 0;
  };
  ...
  const handleExport = () => {
    ...
    const styleHighlight = {
      font: { name: 'Calibri', sz: 10, bold: true, color: { rgb: '3730A3' } },
    ...
    wsData.push([
      { v: 'Metric Name', s: styleHeader },
      { v: 'Category', s: styleHeader },
      { v: 'Unit', s: styleHeader },
      { v: 'Methodology', s: styleHeader },
      { v: '2022', s: styleHeader },
      { v: '2023', s: styleHeader },
      { v: '2024 (Current)', s: { ...styleHeader, fill: { fgColor: { rgb: '6366F1' } } } },
      { v: 'YoY Change', s: styleHeader },
    ]);
    ...
    timeSeriesMetrics.forEach((m, idx) => {
      ...
      wsData.push([
        { v: m.name, s: { ...base, font: { ...base.font, bold: true } } },
        { v: m.category, s: { ...base, alignment: { horizontal: 'center', vertical: 'center' } } },
        { v: m.unit, s: { ...base, alignment: { horizontal: 'center', vertical: 'center' } } },
        { v: m.methodology ?? '', s: base },
        { v: v2022, s: { ...base, alignment: { horizontal: 'right', vertical: 'center' } } },
        { v: v2023, s: { ...base, alignment: { horizontal: 'right', vertical: 'center' } } },
        { v: v2024, s: styleHighlight },
        { v: yoy, s: { ...base, alignment: { horizontal: 'center', vertical: 'center' } } },
      ]);
    });
    ...
    ws['!rows'] = [
      { hpt: 30 }, { hpt: 16 }, { hpt: 8 }, { hpt: 36 },
      ...timeSeriesMetrics.map(() => ({ hpt: 22 })),
    ];
    ws['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 7 } },
      { s: { r: 1, c: 0 }, e: { r: 1, c: 7 } },
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-1/4">Metric Name</TableHead>
                ...
                return (
                  <TableRow key={metric.id} className="hover:bg-slate-50">
                    <TableCell>
                      ...
                      {metric.methodology && (
                        <p className="text-xs text-slate-400 mt-0.5">{metric.methodology}</p>
                      )}
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline" className="bg-white">{metric.category}</Badge>
                    </TableCell>
                    <TableCell className="text-slate-500 text-sm">{metric.unit}</TableCell>
                    <TableCell className="text-right text-slate-600">{formatValue(v2022, metric.unit)}</TableCell>
                    <TableCell className="text-right text-slate-600">{formatValue(v2023, metric.unit)}</TableCell>
                    <TableCell className="text-right font-semibold text-indigo-900 bg-indigo-50/30">
                      {formatValue(v2024, metric.unit)}
                    </TableCell>
                    <TableCell className="text-right">
                      {calculateYoY(v2024, v2023)}
                    </TableCell>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Data Integrity Notes</CardTitle>
          </CardHeader>
          <CardContent>
            <ul className="space-y-3 text-sm text-slate-600">
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5 shrink-0" />
                <p>Data keuangan 2024 berdasarkan laporan keuangan audited yang dirilis pada Maret 2025.</p>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5 shrink-0" />
                <p>Metrik lingkungan (emisi GRK) 2023 telah dikoreksi sesuai pembaruan metodologi GHG Protocol.</p>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5 shrink-0" />
                <p>Metrik sosial (turnover karyawan) tidak termasuk tenaga kerja sementara dan kontrak.</p>
              </li>
            </ul>
          </CardContent>
        </Card>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-slate-600">2024 Data Verified</span>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-slate-600">Historical Data Consistency</span>
              ...
              <div className="pt-4 border-t border-slate-100">
                <p className="text-xs text-slate-500 italic">
                  * Verifikasi dilakukan terhadap dokumen sumber yang ada di Document Vault.
                </p>
              </div>
            </div>
          </CardContent>
